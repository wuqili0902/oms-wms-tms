<template>
  <div>
    <el-card>
      <template #header>
        <div style="display:flex;justify-content:space-between;align-items:center">
          <span>入库管理</span>
          <el-button type="primary" @click="showCreate = true">新建入库单</el-button>
        </div>
      </template>
      <el-table :data="records" stripe v-loading="loading">
        ...
        <el-table-column prop="status" label="状态" width="90">
          <template #default="{ row }"><el-tag :type="row.status === 'RECEIVED' ? 'success' : 'warning'" size="small">{{ row.status }}</el-tag></template>
        </el-table-column>
    ...
    <el-dialog v-model="showCreate" title="新建入库单" width="600px" destroy-on-close @closed="resetForm">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="参考号">
          <el-input v-model.trim="form.ref_no" placeholder="采购单号/调拨单号" />
        </el-form-item>
    ...
    <el-dialog v-model="showDetailDialog" title="入库单详情" width="700px" destroy-on-close>
      ...
      <el-table :data="detail?.lines ?? []" stripe style="margin-top:16px">
        ...
        <el-table-column prop="qty_received" label="数量" />
      </el-table>
    </el-dialog>
  </div>
</template>
...
async function fetchRecords() {
  loading.value = true
  try {
    const res = await apiClient.get(`/stock-in?page=${page.value}&page_size=${pageSize.value}`)
    ...
    ElMessage.error('获取入库记录失败: ' + (e?.response?.data?.detail ?? e.message))
    records.value = []
...
async function submit() {
  ...
  saving.value = true
  try {
    await apiClient.post('/stock-in', { data: { warehouse_id: form.warehouse_id, type: form.type, ref_no: form.ref_no, lines: form.lines } })
...
async function showDetail(row: any) {
  try {
    const res = await apiClient.get(`/stock-in/${row.id}`)
