...
class Warehouse(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    contact: Optional[str] = Column(String(100))
    phone: Optional[str] = Column(String(30))
    config: Optional[dict] = Column(JSON, nullable=True)

    locations: list = relationship("Location", back_populates=True, lazy="dynamic")
    inventory: list = relationship("Inventory", back_populates=True, lazy="dynamic")
    stock_movements: list = relationship("StockMovement", back_populates=True, lazy="dynamic")
    picking_waves: list = relationship("PickingWave", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_warehouses_code", "code",
        unique=True,
    ), Index(
        "ix_warehouses_status", "status",
    ),
...
class Location(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    code: str = Column(String(50), unique=True, index=True)
    zone: Optional[str] = Column(String(20))
    aisle: Optional[str] = Column(String(10))
    shelf: Optional[str] = Column(String(10))
    level: Optional[str] = Column(String(10))
    position: Optional[str] = Column(String(50))
    ...
    warehouse: Warehouse = relationship("Warehouse", back_populates=True)
    inventory_items: list = relationship("Inventory", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_locations_code", "code",
        unique=True,
    ), Index(
        "ix_locations_warehouse_id_zone", "warehouse_id", "zone",
    ),
...
class SKU(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    gtin: Optional[str] = Column(String(13))
    name: Optional[str] = Column(String(200))
    description: Optional[str] = Column(Text)
    category: Optional[str] = Column(String(100))
    manufacturer: Optional[str] = Column(String(200))

    inventory_items: list = relationship("Inventory", back_populates=True, lazy="dynamic")
    order_items: list = relationship("OrderItem", back_populates=True, lazy="dynamic")
...
class Inventory(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    location_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey("locations.id"), nullable=True)
    ...
    warehouse: Warehouse = relationship("Warehouse", back_populates=True)
    location: Optional[Location] = relationship("Location", back_populates=True)
    sku: SKU = relationship("SKU", back_populates=True)
    inventory_logs: list = relationship("InventoryLog", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_inventory_gtin", "gtin",
    ), Index(
        "ix_inventory_batch_no", "batch_no",
    ), Index(
        "ix_inventory_warehouse_id_sku_id", "warehouse_id", "sku_id",
    ),
...
class InventoryLog(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    reference_type: Optional[str] = Column(String(50), nullable=True)
    reference_id: Optional[UUID] = Column(UUID(as_uuid=True), nullable=True)
    operator_id: Optional[UUID] = Column(UUID(as_uuid=True), nullable=True)
    remark: Optional[str] = Column(Text, nullable=True)

    inventory: Inventory = relationship("Inventory", back_populates=True)
    reference: Optional["ReferenceEntity"] = relationship("ReferenceEntity", lazy="dynamic")
    operator: Optional["User"] = relationship("User", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_inventory_logs_inventory_id_created_at", "inventory_id", "created_at",
    ), Index(
        "ix_inventory_logs_reference_type_reference_id", "reference_type", "reference_id",
    ),
...
class ReferenceEntity(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    inventory_logs: list = relationship("InventoryLog", lazy="dynamic")

    __table_args__ = Index(
        "ix_reference_entities_type_id", "reference_type", "reference_id",
        unique=True,
    ),
...
class StockMovement(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    target_warehouse_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey("warehouses.id"), nullable=True)
    source_location_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey("locations.id"), nullable=True)
    target_location_id: Optional[UUID] = Column(UUID(as_uuid=True), ForeignKey("locations.id"), nullable=True)
    sku_id: UUID = Column(UUID(as_uuid=True))
    ...
    target_warehouse: Optional[Warehouse] = relationship("Warehouse", foreign_keys=[target_warehouse_id], lazy="dynamic")
    sku: SKU = relationship("SKU", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_stock_movements_gtin", "gtin",
    ), Index(
        "ix_stock_movements_status_created_at", "status", "created_at",
    ),
...
class PickingWave(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    priority: PickingWavePriority = Column(SAEnum(PickingWavePriority), default=PickingWavePriority.MEDIUM)
    total_items: int = Column(Integer, default=0)
    completed_items: int = Column(Integer, default=0)
    assignee_id: Optional[UUID] = Column(UUID(as_uuid=True), nullable=True)

    warehouse: Warehouse = relationship("Warehouse", back_populates=True)
    orders: list = relationship("Order", back_populates=True, lazy="dynamic")

    __table_args__ = Index(
        "ix_picking_waves_code", "code",
        unique=True,
    ), Index(
        "ix_picking_waves_status_created_at", "status", "created_at",
    ),
